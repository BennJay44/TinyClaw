...
class EnhancedSimpleAgent(SimpleAgent):
    ...
    def _build_messages(self, input_text: str) -> List[Dict]:
        """构建消息列表（保留 tool_calls 和 tool_call_id）

        重写基类方法，确保会话历史中的工具调用消息格式正确。
        """
        messages = []
        ...
        for msg in self._history:
            ...
            if msg.metadata and "tool_calls" in msg.metadata:
                ...
                # 工具调用时 content 应为 null
                if not msg.content:
                    msg_dict["content"] = None

            # 跳过空的 assistant 消息（没有内容也没有工具调用）
            if msg.role == "assistant" and not msg_dict.get("content") and not msg_dict.get("tool_calls"):
                continue
