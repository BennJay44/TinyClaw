"""TinyClaw Agent - 基于 HelloAgents SimpleAgent 的个性化 AI 助手"""
...
from .enhanced_llm import EnhancedHelloAgentsLLM  # TinyClaw 专用 LLM（支持流式工具调用）
...
class TinyClawAgent:
    ...
    def __init__(
        ...
        Args:
            workspace_path: 工作空间路径，默认 ~/.tinyclaw/workspace
        ...
        self.workspace_path = os.path.expanduser(workspace_path or "~/.tinyclaw/workspace")
        ...
        self.name = name or self._read_identity_name() or "TinyClaw"
    ...
    async def _check_and_run_memory_flush(self):
        ...
        if self._memory_flush_manager.should_trigger_flush(estimated_tokens):
            ...
            # 执行静默回合（使用线程池避免阻塞事件循环）
            try:
                response = await asyncio.to_thread(self._agent.run, flush_prompt)
    ...
    def clear_all_history(self):
        ...
        self.name = self._read_identity_name() or "TinyClaw"
