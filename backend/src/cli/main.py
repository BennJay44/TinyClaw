...
@click.group()
@click.version_option(version="0.1.0", prog_name="tinyclaw")
def cli():
    """TinyClaw - 你的个性化 AI 助手"""
    pass
...
def chat(session_id: Optional[str], workspace: Optional[str]):
    ...
    # 确定工作空间路径
    workspace_path = workspace or os.getenv("WORKSPACE_PATH", "~/.tinyclaw/workspace")
    ...
    # 初始化 Agent
    try:
        agent = TinyClawAgent(workspace_path=workspace_path)
...
def config(key: Optional[str], value: Optional[str], workspace: Optional[str], list_all: bool, edit: bool):
    ...
    用法:
      tinyclaw config              # 显示所有配置
      tinyclaw config model_id     # 显示指定配置项
      tinyclaw config model_id glm-4  # 设置配置项
      tinyclaw config --edit       # 用编辑器打开配置文件
    """
    from ..workspace.manager import WorkspaceManager
...
def sessions(workspace: Optional[str], list_all: bool, delete_id: Optional[str], clear: bool):
    ...
    用法:
      tinyclaw sessions           # 列出所有会话
      tinyclaw sessions --list    # 列出所有会话
      tinyclaw sessions --delete <id>  # 删除指定会话
      tinyclaw sessions --clear   # 清除所有会话
    """
    from ..workspace.manager import WorkspaceManager
